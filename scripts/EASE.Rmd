...
geodata_distance<-geodata_duration %>%
  mutate(distance=unlist(lag(lapply(calc_distance,X = 1:6302))))
...
geodata_distance_home <-geodata_distance %>%
  group_by(sub,timeofday) %>%
  summarize(modelat=mlv(lat, method='mfv')[['M']],
            modelon=mlv(lon, method='mfv')[['M']])

homelocation<-ggmap(get_map(location = "Eugene", zoom = 14,
                  maptype = "roadmap", scale = 2)) +
  geom_point(data = geodata_distance_home,
...
# Is the time point a weekday or weekend?




# Determine if participant was stationary or moving
